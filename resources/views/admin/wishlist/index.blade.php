			...
			<table class='table align-middle mb-0'>
				...
				<tbody>
					@forelse($wishlists as $key => $wishlist)
						<tr>
							<td scope='col'>{{ ++$key }}</td>
								<td scope='col'>{{__($wishlist->customer->first_name)}}</td>
							<td scope='col'>{{__($wishlist->product->name)}}</td>
						<td scope='col'>{{__($wishlist->session_id)}}</td>
						<td scope='col'>
							...
							<td>
								<div class="align-items-center gap-3 fs-6">
									<button class="btn bg-gradient-info btn-sm icon-btn ml-1 editBtn"
										...
										data-id="{{ __($wishlist->id)}}"
										data-action="{{ route('admin.wishlist.update',$wishlist->id) }}"
										data-customer="{{ __($wishlist->customer_id)}}"
										data-product="{{ __($wishlist->product_id)}}"
										data-session_id="{{ __($wishlist->session_id)}}">
...
<div id="enableModal" class="modal fade" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">@lang('Wishlist Enable Confirmation')</h5>
				<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form action="{{ route('admin.wishlist.enable') }}" method="POST">
				@csrf
				<input type="hidden" name="id">
				<div class="modal-body">
					<p>@lang('Are you sure to Enable this wishlist?')</p>
				</div>
				<div class="modal-footer">
...
<div id="disableModal" class="modal fade" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">@lang('Wishlist Disable Confirmation')</h5>
				<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form action="{{ route('admin.wishlist.disable') }}" method="POST">
				@csrf
				<input type="hidden" name="id">
				<div class="modal-body">
					<p>@lang('Are you sure to disable this wishlist?')</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-dark" data-bs-dismiss="modal">@lang('No')</button>
					<button type="submit" class="btn btn-primary">@lang('Yes')</button>
...
@push('top-area')
	<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#createModal'><i class='lab la-telegram-plane'></i> @lang('Add Wishlist')</button>
@endpush
@push('js')
	<script>
		(function($){
