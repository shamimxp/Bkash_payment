...
<div class='row mt-3'>
	<div class='col-12 col-lg-12'>
		<div class='card shadow-sm border-0'>
			<div class='card-body'>
				<form action="{{ route('admin.order_details.update', $order_details->id) }}" method="post" enctype="multipart/form-data">
					@csrf
							...
							<div class="row g-3">
								<div class='col-md-12'>
									<div class='form-group mb-3'>
										...
										<select class="form-control " name="order" id="order01" required>
											...
											@foreach($orders as $order)
												<option value="{{ $order->id }}" @if($order->id == $order_details->order_id) selected @endif >{{__($order->order_number) }}</option>
											@endforeach
										</select>
									</div>
									<div class='form-group mb-3'>
										...
										<select class="form-control " name="product" id="product01" required>
											...
											@foreach($products as $product)
												<option value="{{ $product->id }}" @if($product->id == $order_details->product_id) selected @endif >{{__($product->name) }}</option>
											@endforeach
										</select>
									</div>
									<div class='form-row form-group mt-3'>
										...
										<input type="text" class="form-control" name="buying_price" value="{{__($order_details->buying_price)}}" id="buying_price01" required>
									</div>
									<div class='form-row form-group mt-3'>
										<label for="regular_price01" class='font-weight-bold form-label'>@lang('Regular_price')</label><span class='text-danger'>*</span>
										<input type="text" class="form-control" name="regular_price" value="{{__($order_details->regular_price)}}" id="regular_price01" required>
									</div>
									<div class='form-row form-group mt-3'>
										<label for="wholesale_price01" class='font-weight-bold form-label'>@lang('Wholesale_price')</label>
										<input type="text" class="form-control" name="wholesale_price" value="{{__($order_details->wholesale_price)}}" id="wholesale_price01" >
									</div>
									<div class='form-row form-group mt-3'>
										<label for="wholesale_minimum_quantity01" class='font-weight-bold form-label'>@lang('Wholesale_minimum_quantity')</label>
										<input type="text" class="form-control" name="wholesale_minimum_quantity" value="{{__($order_details->wholesale_minimum_quantity)}}" id="wholesale_minimum_quantity01" >
									</div>
									<div class='form-row form-group mt-3'>
										<label for="total_price01" class='font-weight-bold form-label'>@lang('Total_price')</label><span class='text-danger'>*</span>
										<input type="text" class="form-control" name="total_price" value="{{__($order_details->total_price)}}" id="total_price01" required>
									</div>
									<div class='form-row form-group mt-3'>
										<label for="details01" class='font-weight-bold form-label'>@lang('Details')</label>
										<input type="text" class="form-control" name="details" value="{{__($order_details->details)}}" id="details01" >
									</div>
