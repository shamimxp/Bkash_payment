...
<div class='row mt-3'>
	<div class='col-12 col-lg-12'>
		<div class='card shadow-sm border-0'>
			<div class='card-body'>
				<form action="{{ route('admin.coupon_category.update', $coupon_category->id) }}" method="post" enctype="multipart/form-data">
					@csrf
									...
									<div class='form-group mb-3'>
										...
										<select class="form-control " name="coupon" id="coupon01" required>
											...
											@foreach($coupons as $coupon)
												<option value="{{ $coupon->id }}" @if($coupon->id == $coupon_category->coupon_id) selected @endif >{{__($coupon->name) }}</option>
											@endforeach
										</select>
									</div>
									<div class='form-group mb-3'>
										...
										<select class="form-control " name="product" id="product01" required>
											...
											@foreach($products as $product)
												<option value="{{ $product->id }}" @if($product->id == $coupon_category->product_id) selected @endif >{{__($product->name) }}</option>
											@endforeach
										</select>
									</div>
