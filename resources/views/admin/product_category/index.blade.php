			...
			<table class='table align-middle mb-0'>
				...
				<tbody>
					@forelse($product_categories as $key => $product_category)
						<tr>
							<td scope='col'>{{ ++$key }}</td>
								<td scope='col'>{{__($product_category->category->name)}}</td>
							<td scope='col'>{{__($product_category->product->name)}}</td>
						<td scope='col'>
							...
							<td>
								<div class="align-items-center gap-3 fs-6">
									<button class="btn bg-gradient-info btn-sm icon-btn ml-1 editBtn"
										...
										data-id="{{ __($product_category->id)}}"
										data-action="{{ route('admin.product_category.update',$product_category->id) }}"
										data-category="{{ __($product_category->category_id)}}"
										data-product="{{ __($product_category->product_id)}}">
...
@if($product_categories->hasPages())
					...
					<div class="row">
						<div class='col-md-12'>
							<div class='form-row form-group mt-3'>
								<label for="category01" class='font-weight-bold form-label'>@lang('Category')<span class='text-danger'>*</span></label>
								<div class='col-md-12'>
...
<div id="enableModal" class="modal fade" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">@lang('Product_category Enable Confirmation')</h5>
				<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form action="{{ route('admin.product_category.enable') }}" method="POST">
				@csrf
				<input type="hidden" name="id">
				<div class="modal-body">
					<p>@lang('Are you sure to Enable this product_category?')</p>
				</div>
				<div class="modal-footer">
...
<div id="disableModal" class="modal fade" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">@lang('Product_category Disable Confirmation')</h5>
				<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form action="{{ route('admin.product_category.disable') }}" method="POST">
				@csrf
				<input type="hidden" name="id">
				<div class="modal-body">
					<p>@lang('Are you sure to disable this product_category?')</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-dark" data-bs-dismiss="modal">@lang('No')</button>
					<button type="submit" class="btn btn-primary">@lang('Yes')</button>
...
@push('top-area')
	<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#createModal'><i class='lab la-telegram-plane'></i> @lang('Add Product_category')</button>
@endpush
@push('js')
	<script>
		(function($){
			...
			$('.editBtn').on('click',function(){
				...
				editModal.find('select[name=category]').val($(this).data('category'));
