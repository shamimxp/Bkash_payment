							...
							<div class="row g-3">
								<div class='col-md-8'>
									<div class='form-group mb-3'>
										...
										<select class="form-control " name="category" id="category01" required>
											...
											@foreach($categories as $category)
												<option value="{{ $category->id }}" @if($category->id == $sub_category->category_id) selected @endif >{{__($category->name) }}</option>
											@endforeach
										</select>
									</div>
									<div class='form-row form-group mt-3'>
										...
										<input type="text" class="form-control" name="name" value="{{__($sub_category->name)}}" id="name01" required>
									</div>
									<div class='form-row form-group mt-3'>
										<label for="name01" class='font-weight-bold form-label'>@lang('Slug')</label><span class='text-danger'>*</span>
										<input type="text" class="form-control" name="slug" value="{{__($sub_category->slug)}}" id="slug" required>
									</div>
									<div class='form-row form-group mt-3'>
										<label for="meta_title01" class='font-weight-bold form-label'>@lang('Meta Title')</label>
										<input type="text" class="form-control" name="meta_title" value="{{__($sub_category->meta_title)}}" id="meta_title01" >
									</div>
									<div class='form-row form-group mt-3'>
										<label for="meta_description01" class='font-weight-bold form-label'>@lang('Meta Description')</label>
										<textarea class="form-control " rows="5" name="meta_description"><?php echo $sub_category->meta_description; ?></textarea>
									</div>
									<div class='form-row form-group mt-3'>
										...
										<select class="select2-auto-tokenize form-control" name="meta_keywords[]" multiple="multiple">
											@if(@$sub_category->meta_keywords)
												@foreach($sub_category->meta_keywords as $keyword)
									...
									<div class="form-group row pe-0 mt-3">
					                	<div class="form-group col-md-4 pe-0">
											...
											<div class="">
												<input type="checkbox" name="top_category" data-width="100%" data-toggle="toggle" data-onstyle="success" data-offstyle="danger" data-on="Yes" data-off="No" data-size="normal"@if($sub_category->top_category) checked @endif>
											</div>
										</div>
					                	<div class="form-group col-md-4 pe-0">
											<label class="form-label fw-bold">@lang('Special Category')</label>
											<div class="">
												<input type="checkbox" name="special_category" data-width="100%" data-toggle="toggle" data-onstyle="success" data-offstyle="danger" data-on="Yes" data-off="No" data-size="normal"@if($sub_category->special_category) checked @endif>
											</div>
										</div>
					                	<div class="form-group col-md-4 pe-0">
											<label class="form-label fw-bold">@lang('Filter Category')</label>
											<div class="">
												<input type="checkbox" name="filter_category" data-width="100%" data-toggle="toggle" data-onstyle="success" data-offstyle="danger" data-on="Yes" data-off="No" data-size="normal"@if($sub_category->filter_category) checked @endif>
											</div>
										</div>
									</div>
								</div>
                                <div class='col-md-4'>
                                    <div class='form-group'>
                                        <div class='image-upload-area'>
                                            ...
                                            <img id='preview' src="{{ displayImage('assets/classicmart/images/sub_categories/'.$sub_category->image) }}" alt='Image'/>
